...
class LearningProgressServiceTest {
    ...
    private TaskAssignmentService assignmentProxy() {
        return (TaskAssignmentService) Proxy.newProxyInstance(
                ...
                (p, method, args) -> {
                    if ("listAssignedTasks".equals(method.getName())) {
                        return new ArrayList<>(tasks);
                    }
                    if ("list".equals(method.getName())) {
                        return submissions.stream()
                                .map(sub -> {
                                    com.neu.CoursePlatform.entity.TaskAssignment a =
                                            new com.neu.CoursePlatform.entity.TaskAssignment();
                                    a.setTaskNo(sub.getTaskNo());
                                    a.setStudentNo(sub.getStudentNo());
                                    a.setCourseCode("c1");
                                    a.setStatus("active");
                                    return a;
                                })
                    ...
                    if ("countActiveByTaskNo".equals(method.getName())) {
                        return (long) submissions.stream()
                                .filter(s -> args[0].equals(s.getTaskNo())).count();
                    }
                    return List.of();
                });
    }
