...
import {
  ...
  getAbilityRadar, explainKnowledgePoint, askKnowledgePoint, getKnowledgeMastery,
  assignTask, getAssignedTasks, cancelTaskAssignment,
  getTowerRun, generateTowerRun, studentRegister, teacherRegister,
  uploadCourseResource, deleteCourseResource,
  getTowerNode, enterTowerNode, completeTowerNode, diagnoseTowerNode,
  getTowerQuestionPack, getAbilityDeltas,
  // 补充覆盖: 课程列表、学生统计、试卷版本别名、分析接口
  getCourseList, getStudentStats, generateExamVersion,
  generateProblemCluster, getProblemCluster,
  generateStudentIntervention, exportClassReport
} from '../index'
...
describe('补充覆盖 — 能力雷达与知识答疑', () => {
  beforeEach(() => { vi.clearAllMocks() })

  it('getAbilityRadar 传递课程、run、node 参数', async () => {
    await getAbilityRadar('S1', 'C001', 'run1', 'node1')
    expect(mockAxiosInstance.get).toHaveBeenCalledWith('/api/students/S1/ability-radar', {
      params: { course_id: 'C001', run_id: 'run1', node_id: 'node1' }
    })
  })

  it('explainKnowledgePoint 发送 POST 携带数据', async () => {
    await explainKnowledgePoint('KP-1', { question: '解释一下' })
    expect(mockAxiosInstance.post).toHaveBeenCalledWith('/api/knowledge-points/KP-1/explain', { question: '解释一下' })
  })

  it('askKnowledgePoint 发送 POST 到 qa 端点', async () => {
    await askKnowledgePoint('KP-1', { question: '什么是多态' })
    expect(mockAxiosInstance.post).toHaveBeenCalledWith('/api/knowledge-points/KP-1/qa', { question: '什么是多态' })
  })

  it('getKnowledgeMastery 传递学生和课程参数', async () => {
    await getKnowledgeMastery('S1', 'C001')
    expect(mockAxiosInstance.get).toHaveBeenCalledWith('/api/knowledge-mastery/student/S1', { params: { courseCode: 'C001' } })
  })
})

describe('补充覆盖 — 任务分配与注册', () => {
  beforeEach(() => { vi.clearAllMocks() })

  it('assignTask 发送 POST 到分配端点', async () => {
    await assignTask('task01', { studentIds: ['S1', 'S2'] })
    expect(mockAxiosInstance.post).toHaveBeenCalledWith('/api/tasks/task01/assign', { studentIds: ['S1', 'S2'] })
  })

  it('getAssignedTasks 查询已分配任务', async () => {
    await getAssignedTasks('S1', { status: 'assigned' })
    expect(mockAxiosInstance.get).toHaveBeenCalledWith('/api/students/S1/assigned-tasks', { params: { status: 'assigned' } })
  })

  it('cancelTaskAssignment 发送 DELETE', async () => {
    await cancelTaskAssignment('assign-1')
    expect(mockAxiosInstance.delete).toHaveBeenCalledWith('/api/task-assignments/assign-1')
  })

  it('studentRegister 发送 POST 注册', async () => {
    await studentRegister({ username: 'newstu', password: 'pass' })
    expect(mockAxiosInstance.post).toHaveBeenCalledWith('/api/students/register', { username: 'newstu', password: 'pass' })
  })

  it('teacherRegister 发送 POST 注册', async () => {
    await teacherRegister({ username: 'newtch', password: 'pass' })
    expect(mockAxiosInstance.post).toHaveBeenCalledWith('/api/teachers/register', { username: 'newtch', password: 'pass' })
  })
})

describe('补充覆盖 — 塔爬与资源上传', () => {
  beforeEach(() => { vi.clearAllMocks() })

  it('getTowerRun 获取塔爬运行状态', async () => {
    await getTowerRun('S1', 'C001')
    expect(mockAxiosInstance.get).toHaveBeenCalledWith('/api/students/S1/tower-run', { params: { course_id: 'C001' } })
  })

  it('generateTowerRun 生成塔爬运行', async () => {
    await generateTowerRun('S1', 'C001', true)
    expect(mockAxiosInstance.post).toHaveBeenCalledWith('/api/students/S1/tower-run/generate', null, { params: { course_id: 'C001', force: true } })
  })

  it('uploadCourseResource 发送 FormData', async () => {
    const fd = new FormData()
    fd.append('title', '课件')
    await uploadCourseResource(fd)
    expect(mockAxiosInstance.post).toHaveBeenCalledWith('/api/resources', fd, {
      headers: { 'Content-Type': 'multipart/form-data' }
    })
  })

  it('deleteCourseResource 发送 DELETE', async () => {
    await deleteCourseResource('res-1')
    expect(mockAxiosInstance.delete).toHaveBeenCalledWith('/api/resources/res-1')
  })
})

describe('补充覆盖 — 塔爬节点操作', () => {
  beforeEach(() => { vi.clearAllMocks() })

  it('getTowerNode 获取节点详情', async () => {
    await getTowerNode('S1', 'run1', 'node1')
    expect(mockAxiosInstance.get).toHaveBeenCalledWith('/api/students/S1/tower-run/run1/nodes/node1')
  })

  it('enterTowerNode 进入节点', async () => {
    await enterTowerNode('S1', 'run1', 'node1')
    expect(mockAxiosInstance.post).toHaveBeenCalledWith('/api/students/S1/tower-run/run1/nodes/node1/enter')
  })

  it('completeTowerNode 完成节点', async () => {
    await completeTowerNode('S1', 'run1', 'node1', { answers: ['A', 'B'] })
    expect(mockAxiosInstance.post).toHaveBeenCalledWith('/api/students/S1/tower-run/run1/nodes/node1/complete', { answers: ['A', 'B'] })
  })

  it('diagnoseTowerNode 诊断节点', async () => {
    await diagnoseTowerNode('S1', 'run1', 'node1', { errors: 2 })
    expect(mockAxiosInstance.post).toHaveBeenCalledWith('/api/students/S1/tower-run/run1/nodes/node1/diagnose', { errors: 2 })
  })

  it('getTowerQuestionPack 获取题目包', async () => {
    await getTowerQuestionPack('S1', 'run1', 'node1', 'battle')
    expect(mockAxiosInstance.get).toHaveBeenCalledWith('/api/students/S1/tower-run/run1/nodes/node1/question-pack', { params: { mode: 'battle' } })
  })

  it('getAbilityDeltas 获取能力变化', async () => {
    await getAbilityDeltas('S1', 'C001', 'run1')
    expect(mockAxiosInstance.get).toHaveBeenCalledWith('/api/students/S1/ability-deltas', { params: { course_id: 'C001', run_id: 'run1' } })
  })

  it('getAbilityDeltas 默认空 runId 传 undefined', async () => {
    await getAbilityDeltas('S1', 'C001')
    expect(mockAxiosInstance.get).toHaveBeenCalledWith('/api/students/S1/ability-deltas', { params: { course_id: 'C001', run_id: undefined } })
  })

  it('getAbilityRadar 默认空 nodeId 传 undefined', async () => {
    await getAbilityRadar('S1', 'C001')
    expect(mockAxiosInstance.get).toHaveBeenCalledWith('/api/students/S1/ability-radar', {
      params: { course_id: 'C001', run_id: undefined, node_id: undefined }
    })
  })
})

describe('补充覆盖 — 课程与管理模块', () => {
  beforeEach(() => { vi.clearAllMocks() })

  it('getCourseList 获取全部课程列表', async () => {
    await getCourseList()
    expect(mockAxiosInstance.get).toHaveBeenCalledWith('/api/courses/list')
  })
})

describe('补充覆盖 — 提交与统计模块', () => {
  beforeEach(() => { vi.clearAllMocks() })

  it('submitTask 提交任务表单', async () => {
    const formData = { get: vi.fn((key) => key === 'taskNo' ? 'task-1' : null) }
    await submitTask(formData)
    expect(mockAxiosInstance.post).toHaveBeenCalledWith('/api/tasks/task-1/submit', formData, { headers: { 'Content-Type': 'multipart/form-data' } })
  })

  it('getStudentStats 获取学生统计', async () => {
    await getStudentStats('2024001')
    expect(mockAxiosInstance.get).toHaveBeenCalledWith('/api/students/2024001/stats')
  })

  it('generateExamVersion 生成试卷版本 (generatePaper 别名)', async () => {
    await generateExamVersion('CS101', { strategy: 'difficulty' })
    expect(mockAxiosInstance.post).toHaveBeenCalledWith('/api/exams/generate', { strategy: 'difficulty' }, { params: { courseCode: 'CS101' } })
  })

  it('generateProblemCluster 生成问题聚类', async () => {
    await generateProblemCluster('class1', 'CS101')
    expect(mockAxiosInstance.post).toHaveBeenCalledWith('/api/classes/class1/problem-cluster', null, { params: { courseId: 'CS101' } })
  })

  it('getProblemCluster 获取问题聚类结果', async () => {
    await getProblemCluster('class1')
    expect(mockAxiosInstance.get).toHaveBeenCalledWith('/api/classes/class1/problem-cluster')
  })

  it('generateStudentIntervention 生成学生干预方案', async () => {
    await generateStudentIntervention('2024001', 'CS101')
    expect(mockAxiosInstance.post).toHaveBeenCalledWith('/api/students/2024001/intervention', null, { params: { courseId: 'CS101' } })
  })

  it('exportClassReport 导出班级报告', async () => {
    await exportClassReport('class1', { format: 'pdf' })
    expect(mockAxiosInstance.post).toHaveBeenCalledWith('/api/classes/class1/reports/export', { format: 'pdf' })
  })
})
